...
class SuzukiMiyauraModel(BuchwaldHartwigModel):
    ...
    def forward(self, 
        ...
        react1_tensor, react1_scope =  self.mpnn([self.graphs[smi] for smi in data["washed_Organoboron_SMILES"]])
        react2_tensor, react2_scope =  self.mpnn([self.graphs[smi] for smi in data["washed_Organic_Halide_SMILES"]])
