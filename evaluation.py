...
if __name__=="__main__":  
    ...
    parser.add_argument("--seed", default=[0, 5, 33, 42, 49, 51, 53, 62, 65, 97], type=list[int], help="")
    parser.add_argument("--target", default="buchwald-hartwig", type=str, help="")
    parser.add_argument("--extrapolation", default=False, type=bool, help="")
    parser.add_argument("--model_name", defalt="Contrastive", type=str, help="")
    parser.add_argument("--train_size", default=[70, 50, 30, 20, 10, 5.0, 2.5], type=list, help="")
    parse_args = parser.parse_args()

    columns, names = necessary_member(parse_args)
    
    if parse_args.extrapolation:
        ...
        for name in names:
            ...
            for seed in parse_args.seed:
                ...
                predicts.append(np.mean(np.array(predict.loc[199,"test_R2"]).reshape(-1, 1), axis=1))
        ...
        for name in names:
            pre_te_list   = list()
            test = pd.read_csv(f"DataFrame/comparing_data/{name}_test.csv", index_col=0)
            for seed in parse_args.seed:
        ...
        os.makedirs(f"DataFrame/{parse_args.target}/evaluation/{parse_args.model_name}/sample{parse_args.sample_num}/extrapolation/", exist_ok=True)
        result.to_csv(f"DataFrame/{parse_args.target}/evaluation/{parse_args.model_name}/sample{parse_args.sample_num}/extrapolation/ensemble.csv")
